...
def set_up_datasets(args):
    import atari_encode as Dataset
    args.base_class = 101
    args.num_classes = 155 
    args.way = args.episode_way
    args.shot = args.episode_shot
    args.sessions = math.ceil((54)/args.way) + 1
...
class FSCILTrainer():
    ...
    def get_base_dataloader_meta(self):
        txt_path = "data/index_list/" + "atari155_encode/session_" + str(0 + 1) + '.txt'
        
        class_index = np.arange(self.args.base_class)
        trainset = self.args.Dataset.Atari_Encode(root=self.args.dataroot, train=True, index_path=txt_path)
        testset = self.args.Dataset.Atari_Encode(root=self.args.dataroot, train=False, index=class_index)
    ...
    def get_new_dataloader(self, session):
        txt_path = "data/index_list/" + "atari155_encode/session_" + str(session + 1) + '.txt'

        trainset = self.args.Dataset.Atari_Encode(root=self.args.dataroot, train=True,
                                                      index_path=txt_path)
        ...
        testset = self.args.Dataset.Atari_Encode(root=self.args.dataroot, train=False,
                                                      index=class_new)
